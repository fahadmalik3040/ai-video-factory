...
export async function runOmniDistribution(jobIndexStr?: string): Promise<void> {
  ...
  const outMediaFiles = fs.readdirSync(outDir).filter(f => f.endsWith('.mov') || f.endsWith('.mp4'));
  const filesToDistribute: string[] = outMediaFiles.map(f => path.resolve(outDir, f));
