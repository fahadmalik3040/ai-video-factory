...
export const PremiumParticles3D = ({ themeColor = "#00ffcc", particleCount = 25000 }: any) => {
  ...
  return (
    <points ref={pointsRef}>
      ...
      <shaderMaterial 
        uniforms={uniforms}
        vertexShader="attribute float scale; uniform float time; varying float vAlpha; void main() { vec4 mvPosition = modelViewMatrix * vec4(position, 1.0); gl_PointSize = scale * (250.0 / -mvPosition.z) * (1.0 + sin(time + scale * 10.0)*0.5); vAlpha = scale; gl_Position = projectionMatrix * mvPosition; }"
